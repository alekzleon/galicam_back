...
class RegionResource extends JsonResource
{
    public function toArray(Request $request): array
    {
        ...
        return [
            ...
            'stripe_connect' => $this->when($includeTranslations, fn () => [
                'account_id' => $this->stripe_account_id,
                'status' => $this->stripe_connect_status,
                'details_submitted' => (bool) $this->stripe_details_submitted,
                'charges_enabled' => (bool) $this->stripe_charges_enabled,
                'payouts_enabled' => (bool) $this->stripe_payouts_enabled,
                'capabilities' => $this->stripe_capabilities ?? [],
                'requirements' => $this->stripe_requirements ?? [],
                'synced_at' => $this->stripe_synced_at?->toDateTimeString(),
            ]),
            ...
            'products' => $this->whenLoaded('products', fn () => $this->products
                ->map(fn ($product) => [
                    ...
                    'regional_price' => $product->pivot?->regional_price !== null ? (float) $product->pivot->regional_price : null,
                    'stock' => $product->stock !== null ? (float) $product->stock : null,
                    'regional_stock' => $product->pivot?->regional_stock !== null ? (float) $product->pivot->regional_stock : null,
                    'catalog_is_active' => (bool) ($product->pivot?->is_active ?? true),
                    'commission_rate' => $product->pivot?->commission_rate !== null ? (float) $product->pivot->commission_rate : null,
                    'metadata' => $product->pivot?->metadata,
